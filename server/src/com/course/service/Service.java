...
public class Service {
    ...
    public void startServer(){
        ...
        server.addEventListener("list_message",Model_Send_Message.class,new DataListener<Model_Send_Message>() {
            @Override
            public void onData(SocketIOClient sioc, Model_Send_Message t, AckRequest ar) throws Exception {
                try {
                    List<Model_Send_Message> list=serviceUser.getMessage();
        ...
        server.addEventListener("find_message",Model_Send_Message.class,new DataListener<Model_Send_Message>() {
            @Override
            public void onData(SocketIOClient sioc, Model_Send_Message t, AckRequest ar) throws Exception {
                try {
                    List<Model_Send_Message> list=serviceUser.getMessage();
                     sioc.sendEvent("find_message", list.toArray());
                } catch (SQLException e) {
                    System.err.println(e);
                }
            }
        ...
        server.addEventListener("send_to_user", Model_Send_Message.class, new DataListener<Model_Send_Message>() {
            ...
            public void onData(SocketIOClient sioc, Model_Send_Message t, AckRequest ar) throws Exception {
                sendToClient(t);
              Model_Send_Message message=serviceUser.sendMsgToDb(t);
              ar.sendAckData(message.getFromUserID(),message.getToUserID(),message.getText(),message.getDate());
